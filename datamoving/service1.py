    ...
    if select_count == insert_info:
        ...
        for i in range(delete_sql.__len__()):
            if new_tables[i] in delete_sql[i]:
                ...
                # my_print("当前表为:%s 删除数量为:%s" % (new_tables[i], curs.rowcount), log)
        conn_curs.commit()
        # my_print("源表数据删除成功！", log)
...
def execute_dmp_imp(man_f, **global_dict):
    ...
    # total_count = global_dict['total_count']  #总笔数
    table_names = global_dict['table_names']
    tables = global_dict['tables']
    conn = global_dict['conn']
    curs = global_dict['curs']
    ...
    #drop_sql 执行清空临时表的操作
    drop_sql = execute_sh3(conn, table_names, source['db_username'], 'transfer_' + str(task.rule_id), commit_count, '4', sh_path)

    save_sql = execute_sh(conn, table_names, 'transfer_' + str(task.rule_id), source['db_username'], commit_count, '5', sh_path,
        ...
        while True:
            ...
            if FLAG < 1:
                ...
                try:
                    ...
                    truncate_temp(drop_sql, curs, conn_curs, task)
                    # 如果上一次任务有异常，临时表内的内容可能没有删除，在此处先进行一次清表操作
                    # delete_temp_table(drop_sql, curs, task, log)
            if FLAG < 2:
                # 向临时表插入数据(total_count)
                # result = insert_temp_table(create_sql, commit_count, drop_sql, conn_curs, task, log, curs)
            ...
            if FLAG < 3:
                # 分批导入目标库
                total = insert_target_db(insert_sql, commit_count, tables, insert_info, select_count, curs, t_curs, t_conn_curs,
                                 task, log)
                #插入到目标库后，将临时表中已迁移的数据状态改为1
                # update_temp_status(update_sql, task, curs, conn_curs)
                # 导入到目标库中，将导入的数据内容保存到日志表中
                if total != 'break':
            ...
            if FLAG < 4:
                ...
                # update_temp_status(update_sql, task, curs, conn_curs)
            #delete_word = "删除"
                #删除完源库数据后，将删除的数据内容保存到日志表中
            # if int(delete_num) == int(task.total_count):
            #         # 删除临时表内的数据
            #     truncate_temp(drop_sql, curs, conn_curs, task)
            #     my_print('迁移数据量和需要迁移的总量一致', log)
            #     break
            # if int(sum_delete) <= int(total_count):
            #     print('wangxiaogang')
            #print(save_sql, 'save_Sql1111111')
            #execute_save_log_delete(save_sql, curs, count, log, conn_curs, delete_word)
            # 插入到目标库后，将临时表中已迁移的数据状态改为1
            update_temp_status(update_sql, task, curs, conn_curs)
            FLAG = 2
...
def truncate_temp(drop_sql, curs, conn_curs, task):
    """清空临时表"""
    mini_curs(drop_sql[0], curs)
    conn_curs.commit()
    ...
    if count == 1:
        for sql in save_sql:
            if 'create table' in sql:
                ...
                curs = mini_curs(sql, curs)
                conn_curs.commit()
                #my_print('22向TRANSFER_LOG日志表插入记录,' + word + '数据量为：%s ' % curs.rowcount, log)
    ...
    if count == 1:
        for sql in save_sql:
            if 'create table' in sql:
                ...
                curs = mini_curs(sql, curs)
                conn_curs.commit()
                #my_print('11向TRANSFER_LOG日志表插入记录,' + word + '数据量为：%s ' % curs.rowcount, log)
...
def execute_sh3(conn, table_names, tmptablename, dbuser, commitnum,
    ...
    if if_use:
        command = "sh %s %s %s %s %s %s" % (
            sh_path, sh_type(type_num), table_names, dbuser, tmptablename, commitnum)
    else:
        command = "sh %s %s %s %s %s" % (sh_path, sh_type(type_num), tmptablename, dbuser, commitnum)
    result = readline_execute(conn, command)
    limit = []
...
:return: 返回相关sql
    ...
    if if_use:
        command = "sh %s %s %s %s %s %s %s %s %s %s %s %s" % (
            sh_path, sh_type(type_num), table_names, tmptablename, dbuser, commitnum, if_use,
            if_recode, rule_name, task_id, RB_CONDITION, totalcount)
    else:
        command = "sh %s %s %s %s %s %s" % (sh_path, sh_type(type_num), table_names, tmptablename, dbuser, commitnum)
    result = readline_execute(conn, command)
    limit = []
